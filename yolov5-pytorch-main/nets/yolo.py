...
class FeatureNet(nn.Module):
    ...
    def __init__(self, feature_encoder : CSPDarknet, flow_net : FlowNetS, feature_embeddings : list[FeatureEmbedding]):
    ...
    def forward(self, x):
        x = self._feature_aggregation(x)
        return x
    
    def _feature_aggregation(self, frames):
        device = next(self.parameters()).device
        frames = frames.to(device)
        # [tensor(N,C,H,W)]
        feature_maps_tuple = self.feature_encoder(frames)
        ...
        for feature_maps, feature_embedding in zip(feature_maps_tuple, self.feature_embeddings):
            ...
            for i in range(N):
                w_list = []
                f_list = []
                for j in range(max(0, i - FeatureEmbedding.K), min(N, i + FeatureEmbedding.K + 1)):
                    pad_frames = torch.cat([frames[j:j+1], frames[i:i+1]], dim=1)
                    flow_ji = self.flow_net(pad_frames)
                    # 1, c, h, w
                    f_ji = self._feature_warp(feature_maps[j:j+1], flow_ji, device)
                    # 1, emb
...
class YoloBody(nn.Module):
    def __init__(self, anchors_mask, num_classes, phi, pretrained=False, input_shape=[640, 640]):
        ...
        #   初始的基本通道是64
        #-----------------------------------------------#
        backbone = CSPDarknet(base_channels, base_depth, phi, pretrained)
        flow_net = FlowNetS(batchNorm=False, pretrained=True)
        feature_embeddings = [FeatureEmbedding(256, 2048), FeatureEmbedding(512, 2048), FeatureEmbedding(1024, 2048)]
        self.feature_net = FeatureNet(backbone, flow_net, feature_embeddings)
    ...
    def forward(self, x):
        #  backbone
        feat1, feat2, feat3 = self.feature_net(x)
